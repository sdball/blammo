defmodule Mix.Tasks.Gen.SampleLogs do
  ...
  def run(args) do
    ...
    case options do
      {[log_size: log_size_arg], _, _} when not is_nil(log_size_arg) ->
        ...
        generate_sample_logs(log_size, log_size_arg)
  ...
  defp generate_sample_logs(target_size_bytes, target_size) do
    File.mkdir_p!("sample_logs")
    log_path = Path.join(["sample_logs", "sample.#{target_size}.log"])

    File.open!(log_path, [:write], fn file ->
      Enum.reduce_while(Stream.repeatedly(&generate_log_entry/0), 0, fn entry, acc ->
